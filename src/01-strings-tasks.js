...
function extractNameFromTemplate(value) {
  // throw new Error('Not implemented');
  return value.split(',')[1].split(',').toString().slice(1, -1);
}
...
function getRectangleString(width, height) {
  ...
  return header + middle.repeat(height - 2) + footer;
}
...
function encodeToRot13(str) {
  ...
  const index = (x) => inputStr.indexOf(x);
  const translate = (x) => (index(x) > -1 ? outputStr[index(x)] : x);
